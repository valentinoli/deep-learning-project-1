...
def train_model(model, inputs, target, classes = None, learn_rate_= 1e-2, lambda_=0.1, batch_size=100, nb_epochs = 25, auxiliary_loss = False):
    ...
    for e in range(nb_epochs):
        for b in range(0, input_.size(0), batch_size):
            #split the input in two, return the binary value and the classes of both input
            output, class_0, class_1 = model(input_.narrow(0, b, batch_size))
            
            loss = criterion(output, target_.narrow(0, b, batch_size))
            if auxiliary_loss and classes is not None:
                loss += lambda_*criterion(class_0, classes_.narrow(0, b, batch_size)[:,0]) + lambda_*criterion(class_1, classes_.narrow(0, b, batch_size)[:,1])
...
def compute_nb_errors(model, input_, target_, batch_size):
    nb_error = 0
    
    for b in range(0, input_.size(0), batch_size):
        output, _, _ = model(input_.narrow(0, b, batch_size))
        _,pred = output.max(1)
