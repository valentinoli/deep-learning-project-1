...
def grid_search(
    ...
    """
    kfolds = k_fold_split(len(inputs), k=folds)
...
def tune_hyperparameters(num_samples = 1000, folds = 4, verbose=False, start=0, end=5):
    ...
    learning_rates = [0.0002, 0.0005,0.0031,0.012,0.1] #torch.logspace(start=-4, end=-1, steps=5)
    batch_size = 100
    lambdas = [0, 0.115, 0.23, 0.345] #torch.logspace(start=-2, end=0, steps=6)[:-1]
